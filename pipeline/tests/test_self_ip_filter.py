...
async def test_own_ip_events_are_dropped(drive_consumer):
    ...
    pool, r = await drive_consumer([
        _suricata_alert("192.0.2.1", 21, "91.107.118.186", 57889),
        _suricata_alert("192.0.2.1", 21, "91.107.118.186", 57801),
    ])
    assert pool.log == []
    assert r.streams == {}
...
async def test_attacker_events_still_flow(drive_consumer):
    ...
    pool, r = await drive_consumer([
        _suricata_alert("91.107.118.186", 57889, "192.0.2.1", 21),
    ])
    ...
    assert len(events) == 1
    assert events[0]["src_ip"] == "91.107.118.186"
    assert r.streams[ingestor.ENRICH_STREAM] == [{"src_ip": "91.107.118.186"}]
